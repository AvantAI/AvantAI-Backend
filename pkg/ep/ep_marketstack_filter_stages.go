...
// 	gapUpStocks, err := Stage1FilterByGapUpRealTime(apiKey)
// 	if err != nil {
...
// // MarketstackRealTimeResponse for real-time quote
// type MarketstackRealTimeResponse struct {
// 	Data []MarketstackRealTimeData `json:"data"`
// }

// type MarketstackRealTimeData struct {
// 	Symbol        string  `json:"symbol"`
// 	Exchange      string  `json:"exchange"`
// 	Name          string  `json:"name"`
...
// // GetRealTimePremarketData gets the current real-time price and previous close
// // This function should be called 30-40 minutes before market open
// func GetRealTimePremarketData(apiKey string, symbol string) (*MarketstackRealTimeData, *MarketstackEODData, error) {
// 	// Step 1: Get real-time/latest price using the /latest endpoint
// 	// This endpoint provides the most current price available (premarket if before 9:30 AM ET)
// 	realTimeURL := fmt.Sprintf("https://api.marketstack.com/v2/tickers/%s/intraday/latest?access_key=%s",
// 		symbol, apiKey)

// 	fmt.Printf("🔍 Fetching real-time data for %s...\n", symbol)

// 	realTimeResp, err := http.Get(realTimeURL)
// 	if err != nil {
// 		return nil, nil, fmt.Errorf("real-time request failed: %v", err)
// 	}
// 	defer realTimeResp.Body.Close()

// 	realTimeBody, err := io.ReadAll(realTimeResp.Body)
// 	if err != nil {
// 		return nil, nil, fmt.Errorf("failed to read real-time response: %v", err)
// 	}

// 	// Check for API errors
// 	if realTimeResp.StatusCode != 200 {
// 		return nil, nil, fmt.Errorf("API error (status %d): %s", realTimeResp.StatusCode, string(realTimeBody))
// 	}

// 	var realTimeResponse MarketstackRealTimeResponse
// 	if err := json.Unmarshal(realTimeBody, &realTimeResponse); err != nil {
// 		return nil, nil, fmt.Errorf("failed to parse real-time JSON: %v", err)
// 	}

// 	if len(realTimeResponse.Data) == 0 {
// 		return nil, nil, fmt.Errorf("no real-time data available for %s", symbol)
// 	}

// 	log.Printf("Premarket data for %s: %+v\n", symbol, realTimeResponse.Data[0])

// 	currentData := &realTimeResponse.Data[0]

// 	// Step 2: Get previous day's EOD close
// 	eodURL := fmt.Sprintf("https://api.marketstack.com/v2/eod/latest?access_key=%s&symbols=%s",
// 		apiKey, symbol)

// 	eodResp, err := http.Get(eodURL)
// 	if err != nil {
// 		return nil, nil, fmt.Errorf("EOD request failed: %v", err)
// 	}
// 	defer eodResp.Body.Close()

// 	eodBody, err := io.ReadAll(eodResp.Body)
// 	if err != nil {
// 		return nil, nil, fmt.Errorf("failed to read EOD response: %v", err)
// 	}

// 	var eodResponse MarketstackEODResponse
// 	if err := json.Unmarshal(eodBody, &eodResponse); err != nil {
// 		return nil, nil, fmt.Errorf("failed to parse EOD JSON: %v", err)
// 	}

// 	if len(eodResponse.Data) == 0 {
// 		return nil, nil, fmt.Errorf("no EOD data available for %s", symbol)
// 	}

// 	fmt.Printf("End of day data for %s: %+v\n", symbol, eodResponse.Data[0])

// 	previousData := &eodResponse.Data[0]

// 	fmt.Printf("✅ %s: Current Open=$%.2f, PrevClose=$%.2f, Time=%s\n",
// 		symbol, currentData.Open, previousData.Close, currentData.LastTradeTime)

// 	return currentData, previousData, nil
// }

// // Alternative: Use intraday endpoint with time range for premarket
// func GetPremarketDataWithTimeRange(apiKey string, symbol string) (*MarketstackRealTimeData, *MarketstackEODData, error) {
// 	// Get today's date and premarket time range
// 	loc, _ := time.LoadLocation("America/New_York")
// 	now := time.Now().In(loc)
// 	today := now.Format("2006-01-02")

// 	// Premarket hours: 4:00 AM - 9:30 AM ET
// 	// Get the latest intraday data from today
// 	intradayURL := fmt.Sprintf("https://api.marketstack.com/v2/intraday?access_key=%s&symbols=%s&date_from=%s&date_to=%s&interval=1min&sort=DESC&limit=1",
// 		apiKey, symbol, today, today)

// 	fmt.Printf("🔍 Fetching premarket intraday data for %s...\n", symbol)

// 	intradayResp, err := http.Get(intradayURL)
// 	if err != nil {
// 		return nil, nil, fmt.Errorf("intraday request failed: %v", err)
// 	}
// 	defer intradayResp.Body.Close()

// 	intradayBody, err := io.ReadAll(intradayResp.Body)
// 	if err != nil {
// 		return nil, nil, fmt.Errorf("failed to read intraday response: %v", err)
// 	}

// 	if intradayResp.StatusCode != 200 {
// 		return nil, nil, fmt.Errorf("API error (status %d): %s", intradayResp.StatusCode, string(intradayBody))
// 	}

// 	var intradayResponse MarketstackIntradayResponse
// 	if err := json.Unmarshal(intradayBody, &intradayResponse); err != nil {
// 		return nil, nil, fmt.Errorf("failed to parse intraday JSON: %v", err)
// 	}

// 	if len(intradayResponse.Data) == 0 {
// 		return nil, nil, fmt.Errorf("no intraday data available for %s", symbol)
// 	}

// 	// Convert intraday data to real-time format
// 	latestBar := intradayResponse.Data[0]
// 	currentData := &MarketstackRealTimeData{
// 		Symbol:        latestBar.Symbol,
// 		Exchange:      latestBar.Exchange,
// 		Last:          latestBar.Last,
// 		Open:          latestBar.Open,
// 		High:          latestBar.High,
// 		Low:           latestBar.Low,
...
// 	fmt.Printf("✅ %s: Premarket Open=$%.2f, PrevClose=$%.2f, Time=%s\n",
// 		symbol, currentData.Open, previousData.Close, currentData.Date)

// 	return currentData, previousData, nil
// }

// // Updated Stage 1 function to use real-time premarket data
// func Stage1FilterByGapUpRealTime(apiKey string) ([]StockData, error) {
// 	symbols, err := getStockSymbols()
// 	if err != nil {
// 		return nil, fmt.Errorf("failed to get stock symbols: %v", err)
// 	}

// 	fmt.Printf("📈 Processing %d symbols for REAL-TIME gap up analysis...\n", len(symbols))
...
// 	isPremarket := totalMinutes >= 240 && totalMinutes < 570 // 4:00 AM to 9:30 AM

// 	if !isPremarket {
// 		fmt.Printf("⚠️ WARNING: Not in premarket hours (4:00-9:30 AM ET). Current time: %s ET\n", now.Format("15:04"))
// 		fmt.Println("📊 Results may not reflect true premarket activity.")
// 	} else {
// 		fmt.Printf("✅ In premarket hours. Current time: %s ET\n", now.Format("15:04"))
// 	}
...
// 				fmt.Printf("   📊 Progress: %d/%d symbols (Errors: %d, No Data: %d)\n",
// 					processedCount, len(symbols), errorCount, noDataCount)
// 			}
// 			mu.Unlock()

// 			// Try real-time endpoint first
// 			currentData, eodData, err := GetRealTimePremarketData(apiKey, sym)
// 			if err != nil {
// 				// Fallback to intraday with time range
// 				currentData2, eodData2, err2 := GetPremarketDataWithTimeRange(apiKey, sym)
// 				if err2 != nil {
// 					mu.Lock()
// 					errorCount++
// 					mu.Unlock()
// 					if processedCount%100 == 0 {
// 						fmt.Printf("⚠️ Error getting data for %s: %v (fallback also failed: %v)\n", sym, err, err2)
// 					}
// 					return
// 				}
...
// 			if eodData == nil || currentData == nil {
// 				mu.Lock()
// 				noDataCount++
// 				mu.Unlock()
// 				return
...
// 			if eodData.Close <= 0 || currentData.Last <= 0 {
// 				mu.Lock()
// 				noDataCount++
// 				mu.Unlock()
// 				return
...
// 			// Calculate gap up: (Current Price - Previous Close) / Previous Close * 100
// 			// Use "Open" price as the projected opening price for current day (calculated by Marketstack)
// 			currentPrice := currentData.Open
// 			if currentPrice <= 0 {
// 				// Fallback to last trade if open is not available
...
// 			gapUp := ((currentPrice - eodData.Close) / eodData.Close) * 100

// 			// Log interesting cases for debugging
// 			if math.Abs(gapUp) > 5.0 || processedCount%100 == 0 {
// 				fmt.Printf("🔍 %s: Current=$%.2f, PrevClose=$%.2f, Gap=%.2f%%, Time=%s\n",
// 					sym, currentPrice, eodData.Close, gapUp, currentData.LastTradeTime)
// 			}
...
// 					Low:                        currentData.Low,
// 					Close:                      currentPrice, // Use current/premarket price
// 					Volume:                     int64(currentData.Volume),
// 					PreviousClose:              eodData.Close,
// 					Change:                     currentPrice - eodData.Close,
// 					ChangePercent:              gapUp,
// 					ExtendedHoursQuote:         currentPrice,
// 					ExtendedHoursChange:        currentPrice - eodData.Close,
...
// 				mu.Lock()
// 				fmt.Printf("✅ %s QUALIFIES! Gap: %.2f%% (Current: $%.2f, Prev: $%.2f) [%s]\n",
// 					sym, gapUp, currentPrice, eodData.Close, currentData.LastTradeTime)
...
// 	fmt.Printf("\n📈 Final Real-Time Statistics:\n")
// 	fmt.Printf("   Total Processed: %d\n", processedCount)
// 	fmt.Printf("   Errors: %d\n", errorCount)
// 	fmt.Printf("   No Data: %d\n", noDataCount)
// 	fmt.Printf("   Qualified Stocks: %d\n", len(gapUpStocks))

// 	// Sort by gap up percentage (highest first)
// 	sort.Slice(gapUpStocks, func(i, j int) bool {
// 		return gapUpStocks[i].ChangePercent > gapUpStocks[j].ChangePercent
// 	})
...
// 			currentData, eodData, err := getPremarketDataV2(apiKey, sym)
// 			if err != nil {
// 				mu.Lock()
// 				errorCount++
// 				mu.Unlock()
// 				if processedCount%100 == 0 {
// 					fmt.Printf("⚠️ Error getting data for %s: %v\n", sym, err)
// 				}
// 				return
// 			}
...
// 			if eodData.Close <= 0 || currentData.Open <= 0 {
// 				mu.Lock()
// 				noDataCount++
// 				mu.Unlock()
// 				return
...
// 			// Calculate gap up: (Current Price - Previous Close) / Previous Close * 100
// 			// Use the "open" from intraday as the current/expected opening price
// 			gapUp := ((currentData.Open - eodData.Close) / eodData.Close) * 100

// 			// Log interesting cases for debugging
// 			if math.Abs(gapUp) > 5.0 || processedCount%100 == 0 {
// 				fmt.Printf("🔍 %s: Current=$%.2f, PrevClose=$%.2f, Gap=%.2f%%\n",
// 					sym, currentData.Open, eodData.Close, gapUp)
// 			}
...
// 					Volume:                     int64(currentData.Volume),
// 					PreviousClose:              eodData.Close,
// 					Change:                     currentData.Open - eodData.Close,
// 					ChangePercent:              gapUp,
// 					ExtendedHoursQuote:         currentData.Open,
// 					ExtendedHoursChange:        currentData.Open - eodData.Close,
// 					ExtendedHoursChangePercent: gapUp,
// 					Exchange:                   currentData.Exchange,
// 					Name:                       sym,
// 				}
...
// 				mu.Lock()
// 				fmt.Printf("✅ %s QUALIFIES! Gap: %.2f%% (Current: $%.2f, Prev: $%.2f)\n",
// 					sym, gapUp, currentData.Open, eodData.Close)
// 				gapUpStocks = append(gapUpStocks, stockData)
// 				mu.Unlock()
// 			}
// 		}(symbol)
...
// 	// Step 1: Get the latest EOD data (previous day's close)
// 	// Note: No need to specify exchange - API will return data from all exchanges
// 	eodURL := fmt.Sprintf("https://api.marketstack.com/v2/eod/latest?access_key=%s&symbols=%s",
// 		apiKey, symbol)

// 	eodResp, err := http.Get(eodURL)
// 	if err != nil {
// 		return nil, nil, fmt.Errorf("EOD HTTP request failed: %v", err)
// 	}
// 	defer eodResp.Body.Close()
...
// 	// Check for API errors
// 	if strings.Contains(string(eodBody), "error") {
// 		var apiError map[string]interface{}
// 		if err := json.Unmarshal(eodBody, &apiError); err == nil {
// 			return nil, nil, fmt.Errorf("API error: %v", apiError)
// 		}
// 	}
...
// 	// Step 2: Get the latest intraday data (current/premarket price)
// 	// Using /intraday/latest endpoint with after_hours=true to get premarket data
// 	intradayURL := fmt.Sprintf("https://api.marketstack.com/v2/intraday/latest?access_key=%s&symbols=%s",
// 		apiKey, symbol)

// 	intradayResp, err := http.Get(intradayURL)
// 	if err != nil {
// 		// If intraday fails, return EOD data with current price = previous close (no gap)
// 		fmt.Printf("⚠️ Intraday request failed for %s, assuming no gap: %v\n", symbol, err)
// 		placeholderIntraday := &MarketstackIntradayData{
// 			Open:     eodResponse.Data[0].Close,
// 			High:     eodResponse.Data[0].Close,
// 			Low:      eodResponse.Data[0].Close,
// 			Close:    eodResponse.Data[0].Close,
// 			Volume:   0,
// 			Date:     eodResponse.Data[0].Date,
// 			Symbol:   symbol,
// 			Exchange: eodResponse.Data[0].Exchange,
// 		}
// 		return placeholderIntraday, &eodResponse.Data[0], nil
// 	}
// 	defer intradayResp.Body.Close()
...
// 	// Check for API errors
// 	if strings.Contains(string(intradayBody), "error") {
// 		fmt.Printf("⚠️ Intraday API error for %s, using EOD close as current price\n", symbol)
// 		placeholderIntraday := &MarketstackIntradayData{
// 			Open:     eodResponse.Data[0].Close,
// 			High:     eodResponse.Data[0].Close,
// 			Low:      eodResponse.Data[0].Close,
...
// 	if len(intradayResponse.Data) == 0 {
// 		// No intraday data available - use previous close as current price
// 		fmt.Printf("⚠️ No intraday data for %s, using previous close\n", symbol)
// 		placeholderIntraday := &MarketstackIntradayData{
// 			Open:     eodResponse.Data[0].Close,
// 			High:     eodResponse.Data[0].Close,
// 			Low:      eodResponse.Data[0].Close,
...
// 	// Return the most recent intraday data and EOD data
// 	return &intradayResponse.Data[0], &eodResponse.Data[0], nil
// }

// func getPremarketDataV2WithDate(apiKey string, symbol string) (*MarketstackIntradayData, *MarketstackEODData, error) {
// 	// Get yesterday's date for EOD
// 	yesterday := time.Now().AddDate(0, 0, -1).Format("2006-01-02")

// 	// Get EOD data for yesterday
// 	eodURL := fmt.Sprintf("https://api.marketstack.com/v2/eod/%s?access_key=%s&symbols=%s",
// 		yesterday, apiKey, symbol)
...
// 	if len(eodResponse.Data) == 0 {
// 		return nil, nil, fmt.Errorf("no EOD data for %s on %s", symbol, yesterday)
// 	}

// 	// Get today's intraday data
// 	today := time.Now().Format("2006-01-02")
// 	intradayURL := fmt.Sprintf("https://api.marketstack.com/v2/intraday?access_key=%s&symbols=%s&date_from=%s&date_to=%s&limit=1&sort=DESC",
// 		apiKey, symbol, today, today)

// 	intradayResp, err := http.Get(intradayURL)
// 	if err != nil {
// 		// Return EOD close as current price if intraday fails
// 		placeholderIntraday := &MarketstackIntradayData{
// 			Open:     eodResponse.Data[0].Close,
// 			Close:    eodResponse.Data[0].Close,
// 			High:     eodResponse.Data[0].Close,
...
// 	if len(intradayResponse.Data) == 0 {
// 		// No intraday data - use EOD close
// 		placeholderIntraday := &MarketstackIntradayData{
// 			Open:     eodResponse.Data[0].Close,
// 			Close:    eodResponse.Data[0].Close,
// 			High:     eodResponse.Data[0].Close,
