...
func fetchIntraday(apiKey, symbol, fromISO, toISO string) (MarketstackIntradayResponse, error) {
	url := fmt.Sprintf(
		"https://api.marketstack.com/v2/intraday?access_key=%s&symbols=%s&interval=1min&date_from=%s&date_to=%s&limit=1000",
		apiKey, symbol, fromISO, toISO,
	)
...
func intradayWorker(apiKey, symbol, date string, goroutineId int) {
	...
	for {
		...
		// Fetch from session open to now (UTC range)
		fromISO := toISOUTC(openNY)
		toISO := toISOUTC(nowNY)

		resp, err := fetchIntraday(apiKey, symbol, fromISO, toISO)
		if err != nil {
